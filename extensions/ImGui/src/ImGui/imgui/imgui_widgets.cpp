...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        //const bool is_paste = (Shortcut(ImGuiKey_LeftCtrl | ImGuiKey_V, id, f_repeat) || Shortcut(ImGuiMod_Shift | ImGuiKey_Insert, id, f_repeat)) && !is_readonly;
        const bool is_copy  = IsKeyPressed(ImGuiKey_LeftCtrl, true) || IsKeyPressed(ImGuiKey_C, true) && !is_readonly;
        const bool is_paste = IsKeyPressed(ImGuiKey_LeftCtrl, true) || IsKeyPressed(ImGuiKey_V, true) && !is_readonly;
